Unit shapes;
//to be clear: "shape objects" not "window shapes"
...
If we RenderPut every pixel(like in circle drawing)- this takes too much time.
	Do that on a backbuffer instead and pause rendering/input until done.
	Further- optimize compile the code w compiler directives

Also floodfills are "x*y pixel array filled" and will have to be converted to SDL_Fills
...
FIXME:
  ...
  merge with SDL2_gfx unit(and get -a lot of - somethings working)
...
//because of my changes(good)- both JEDI port and SDL_GPU_ need to be rewritten.
