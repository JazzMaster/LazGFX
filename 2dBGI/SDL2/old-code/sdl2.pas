...
(use: GL,GLU,freeGLUT)
  ...
  arising from the use of this software(including screw up by the FP compiler-ruining your OS install).
...
//SDL v1.2 had a lot of old references, such as DOS, win16(3.11) and OS9
//THESE CANNOT BE USED with version 2.x and have been removed.
//This includes older compiler checks

//if using XE or RAD or DELPHI??
{$IFNDEF FPC}
  {$IFDEF Debug}
    //align data and turn on range,stack,io checks plus debug tracers

    {D+,L+,R+,I+,S+,A+}
  {$ELSE}
    //align data and turn on range,stack,io checks.
    {R+,S+,I+,A+}
  {$ENDIF}
{$ELSE}
  {$MODE DELPHI} 
  //dont force delphi mode unless using it- JEDI is slanted torawrds that, which is wrong.
{$ENDIF}
...
//build fails if X11 not linked in.
{$IFDEF UNIX}
	uses
		x,xlib,ctypes;
{$ENDIF}
...
//-but in ctypes pascal unit already
type
...
//dont you want double precision?
  Float = Single;
  {$EXTERNALSYM Float}
...
//from "sdl_timer.h"
   ...
   *  This value wraps if the program runs for more than ~49 days.
	  ...
	  (GetTickCount suffers the same fate)
   *}
...
//pallettes are max 256 colors in nature

  PSDL_Palette = ^TSDL_Palette;
  TSDL_Palette = record
    ncolors: SInt32;
    colors: array [0..255] of PSDL_Color;  //SDL wiki: colors:^SDL_Color -an array of SDL_Color(s) representing the palette
    version: UInt32;
    refcount: SInt32;
  end;

  PSDL_Colors=^TSDL_Colors;
  TSDL_Colors = array [0..255] of PSDL_Color;
...
function SDL_SetPaletteColors(palette: PSDL_Palette; colors: TSDL_Colors; firstcolor: SInt32; ncolors: SInt32): SInt32 cdecl; external SDL_LibName {$IFDEF DELPHI} {$IFDEF MACOS} name '_SDL_SetPaletteColors' {$ENDIF} {$ENDIF};
...
procedure SDL_PauseAudioDevice(dev: TSDL_AudioDeviceID; pause_on: Integer) cdecl; external SDL_LibName {$IFDEF DELPHI} {$IFDEF MACOS} name '_SDL_PauseAudioDevice' {$ENDIF} {$ENDIF};
   ...
   *}
function SDL_LoadWAV_RW(src: PSDL_RWops; freesrc: Integer; spec: PSDL_AudioSpec; audio_buf: PUInt8; audio_len: PUInt32): PSDL_AudioSpec cdecl; external SDL_LibName {$IFDEF DELPHI} {$IFDEF MACOS} name '_SDL_LoadWAV_RW' {$ENDIF} {$ENDIF};
   ...
   *}
   ...
   function SDL_LoadWAV(_file: PAnsiChar; spec: PSDL_AudioSpec; audio_buf: PUInt8; audio_len: PUInt32): PSDL_AudioSpec;
...
type
   ...
   *}
const
  SDL_DL_FLIP_NONE       = $0; {**< Do not flip *}
...
const
  ...
  function SDL_GetEventState(type_: UInt32): UInt8; cdecl; external SDL_LibName {$IFDEF DELPHI} {$IFDEF MACOS} name '_SDL_GetEventState' {$ENDIF} {$ENDIF};
 ...
 * 
 *  -Problem is: SDL2 sets NO PARACHUTE flag by default now.
 *  I have the needed code -but its not here yet.
 *}
