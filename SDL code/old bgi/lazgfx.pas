...
createTextureFromSurface must use surface^.format -of chosen bpp- or it could be off
        however
we already know the bpp- so why bother with surface probes for texture ops?
    just do the color conversion (shifting) already!
...
You need to know:
        ...
        event-driven input and rendering loops (forget the console and ReadKEY)
            further- where they fail miserably- to work.
...
FONTS:
    ...
    SDL_gfx has a 8x8 font embedded into the code headers. 
        THis is a *BITCH* to convert- but allows bitmapped fonts(dont scale well)

This unit is Useful when you need VESA modes but cant have them because X11, etc. is running.
...
CGA as implemented here is actually EGA/VGA.
True CGA is 4plane-4color mode (and its butt ugly).

I didnt have it at the time- but I do now.
...
USER FIXME: SDL and OpenGL can fire errors with no debugging info and leave you clueless as 
    to where to start debugging.(c units not compiled w debug support)
...
4bit(16)->8bit(256):
    This is a faked 32bit palette of [x] entries.
...
SDL bug:
    "You must render in the same (mainloop) that handles input."
    (this comment is mis-understood)
...
//FPC generic units(OS independent)
  SDL2,SDL2_Image,SDL2_mixer,SDL2_TTF,strings,typinfo,logger
...
//to get the longword
procedure RGBAToDWord(somecolor:SDL_Color):DWord;
var
    someDword:DWord;
...
procedure DWordToRGBA(someword:DWord):SDL_Color;
...
begin
    ...
    somecolor^.Red := (someword and $FF000000);
    somecolor^.Green := ((someword and $00FF0000) shr 8);
    somecolor^.Blue := ((someword  and $0000FF00) shr 16);
    somecolor^.Alpha := ((someword and $000000FF) shr 24);
